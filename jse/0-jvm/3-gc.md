...
#### ParNew收集器
是Serial收集器的多线程版本。是Server模式下的虚拟机首选的新生代收集器。
...
Parallel Scavenge收集器的目标则是打到一个可控制的**吞吐量**。（CMS等收集器的关注点是尽可能缩短垃圾收集时用户线程的停顿时间）
...
“吞吐量优先收集器”终于有了比较名副其实的应用组合，在注重吞吐量以及CPU资源敏感的场合，都可以优先考虑Parallel Scavenge收集器+Parallel Old收集器的组合。
...
#### CMS(并发GC)收集器 Concurrent Mark Sweep     !!!!
 以获取最短回收停顿时间为目标的收集器。CPU 资源非常充裕，建议使用 CMS 收集器。
...
#### G1收集器
 G1(Garbage First)收集器是JDK8提供的一个新收集器，在**JDK9中默认使用**，G1收集器基于“标记-整理”算法实现，也就是说不会产生内存碎片。  
 G1收集器有以下特点：  
